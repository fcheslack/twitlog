...
Twitconnect.prototype.tweetlinkCallback = function(tweet){
    winston.info("Received streamed tweet - checking for links");
    try{
        if(tweet.entities){
            _.each(tweet.entities.url, function(val, index){
                winston.info(tweet.created_at + ' : ' + 'Link tweeted: ' + val.url + ' : ' + val.display_url);
            });
        }
    ...
    twit.verifyCredentials(function(err, data){
        ...
        twit.stream(streamtype, streamargs, function(stream) {
            ...
            stream.on('end', function (response) {
                ...
                //winston.info(response);
                tc.config.wait = tc.config.wait * 2;
                if(tc.config.wait > 120000){
                    winston.info("RECONNECT TIMER HIT 30 SECONDS, NOT RECONNECTING");
                    startPolling(pollDelay);
                }
                else{
                    setTimeout(tc.startListening, tc.config.wait);
                }
            });
